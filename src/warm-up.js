...
pages = Math.round(pages - pages * 0.25);
...
let sequel = `${title} and the Chamber of Secrets`;
...
let isShortStory = function (pages) {
  if (pages < 25) {
    return true;
  } else {
    return false;
...
let calculateReadingTime = function (pages) {
  if (pages < 1000) {
...
let talkAboutBook = function (isFavorite) {
  if (isFavorite) {
